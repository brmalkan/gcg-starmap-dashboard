...
def highSigSrcs(radiusCut, sigmaCut, rootDir='./', poly='polyfit_d/fit',
    ...
    tab = Table.read(fitFile, format='ascii', header_start=1)

    name = tab['star_name']
    radius = tab['mag_0'] * scale
    acc = tab['mag_2'] * scale
    accErr = tab['mag_sig_2'] * scale
    sigma = acc / accErr
...
def velVsAcc():
    ...
    name = tab['star_name']
    radius = tab['mag_0'] * scale
    velPhi = tab['phi_1'] * scale
    velRad = tab['mag_1'] * scale
    velPhiErr = tab['phi_sig_1'] * scale
    velRadErr = tab['mag_sig_1'] * scale
    acc = tab['mag_2'] * scale
    accErr = tab['mag_sig_2'] * scale
...
def plotStar(star, root='./', align = 'align/align_d_rms_1000_abs_t',
    ...
    _tabPoints = starTables.read_points(pointsFile)
    date = _tabPoints['epoch']
    x = _tabPoints['x'] * -1.0
    y = _tabPoints['y']
    xerr = _tabPoints['xerr']
    yerr = _tabPoints['yerr']

    _fitPoints = starTables.read_points(fitFile)
    date_f = _fitPoints['epoch']
    x_f = _fitPoints['x'] * -1.0
    y_f = _fitPoints['y']
    xerr_f = _fitPoints['xerr']
    yerr_f = _fitPoints['yerr']
