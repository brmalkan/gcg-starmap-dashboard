...
def newSources(root='./', align='align/align_d_rms_t', poly='polyfit_d/fit',
               orbitFile='/u/ghezgroup/data/gc/source_list/orbits_movie.dat',
               flatten=False,yrlyPts=False):
...
def orbitsAnimate(years=None, rad=0.5, orbitFile='',
                  root='./', align='align/align_d_rms_1000_abs_t',
                  poly='polyfit_c/fit', outdir='/g/ghez/animations/',
                  extendS02=False, plotFullTrails=True,
                  astVsRV=False, showAfterDscvr=False, addG2=False, addG1=False, dpi=100):
    """
    Optional Input:
    years (float):         Number of years for the movie to run. Default is S0-2's period.
    rad (float):           Radial extent of movie (default=0.5 arcsec).
    orbitFile (str):       File with all the orbital elements. Must include
    		    	   full path to the file.
    root (str):            Root directory up to align files.
    extendS02 (bool):	   Set to True to add a series of frames at end of movie
    			   emphasizing the stars that have full orbital coverage
                           to date, which are S0-2 and S0-102 (as of April 2012).
    plotFullTrails (bool): Plot the full orbital trail, even before the star
    			   was discovered. If False, then start plotting trail
                           at the point of discovery in our images.
    astVsRV (bool):        If set to True, plots solid lines for times we had both
    			   spectroscopy and astrometry, and plots dashed lines when
                           we only had astrometry.
    showAfterDscvr (bool): If set to True, stars appear only after they were discovered
    			   at Keck.
    """
    ...
    #tab = asciidata.open('/u/syelda/research/forAndrea/orbitsAnim_120420_crafoord/v1/orbits_movie_crafoord.dat')
    ...
    # If we're including G2, I made a dummy-placeholder in the stars object
    # since G2 isn't detected in our Kp data and so won't be in the align output.
    # I set it to have IRS16C's properties in getOrbitStars(). So let's change those now.
    # NOTE: this will be a problem if we ever want to include 16C in the animation also.
    # Also do the same thing for G1.
    if addG2 == True:
        pdb.set_trace()
        idx = name.index('irs16C')
        mag[idx] = 17
        name[idx] = 'S0-102'
    ...
    gotRV = tab[10].tonumpy()  # Date of first RV measurement ### NEW
    xshift1 = tab[11].tonumpy()    # Shifts for labels (in first frame)
    yshift1 = tab[12].tonumpy()
    xshift2 = tab[13].tonumpy()    # Shifts for labels (in last frame)
    yshift2 = tab[14].tonumpy()
    #pdb.set_trace()
    colors = [tab[15][ss].strip() for ss in range(tab.nrows)]
    ...
    if extendS02 == True:
        fullOrbs = ['S0-2']
        #fullOrbs = ['S0-2', 'S0-38', 'S0-102']

    # Array of time steps (0.1 yr steps)
    t = na.arange(1995.5, 1995.5+years, 0.1, type=na.Float)
    ...
    for tt in range(len(t)):
        ...
        for ss in range(len(s.stars)):
            if showAfterDscvr == True:
                if (time < discovered[ss]):
                    #pdb.set_trace()
                    continue
            star = s.stars[ss]
        ...
        def plotTrails1(extS02=extS02, last=False):
            # Plot the trails for each star 
            for ss in range(len(s.stars)):
                star = s.stars[ss]

                if (extS02 == True):
                    if star.name not in fullOrbs:
                        continue
                ...
                if (tt == 0):
                    ...
                    pylab.text(star.xanim[tt]+xshift1[ss],
                               ...
                               name[ss],color=colors[ss], fontsize=10)
                if ((tt == (len(t)-1)) & (extendS02==False)):
                    pylab.text(star.xanim[tt]+xshift2[ss],
                               star.yanim[tt]+yshift2[ss],
                               name[ss],color=colors[ss], fontsize=10)
                if ((last == True) & (extendS02==True) & (name[ss] in fullOrbs)):
                    pylab.text(star.xanim[tt]+xshift2[ss],
                               star.yanim[tt]+yshift2[ss],
                               name[ss],color=colors[ss], fontsize=10)
                    pdb.set_trace()
        ...
        def plotTrails2(extS02=extS02, last=False):
            # Plot the trails for each star, but only start the trail after the
            # point of discovery
            for ss in range(len(s.stars)):
                star = s.stars[ss]
                ...
                # Solid line since discovery; if accounting for RV measurements,
                # use solid line for when we had both RV and astrometry and
                # dashed line for when we had astrometry only.
                if astVsRV == True:
                    both = where((t < time) & (t >= discovered[ss]) & (t >= gotRV[ss]) & (t <= today))[0]
                    ast = where((t < time) & (t >= discovered[ss]) & (t <= today))[0]
                    #pdb.set_trace()

                    if (len(ast) > 0):
                        pylab.plot(star.xanim[ast], star.yanim[ast], '--',
                                   color=colors[ss], linewidth=2)
                    if (len(both) > 0):
                        pylab.plot(star.xanim[both], star.yanim[both], '-',
                                   color=colors[ss], linewidth=2)
                else:
                    ast = where((t < time) & (t >= discovered[ss]) & (t <= today))[0]
                    if (len(ast) > 0):
                ...
                if ((tt == (len(t)-1)) & (extendS02==False)):
                    pylab.text(star.xanim[tt]+xshift2[ss],
                               star.yanim[tt]+yshift2[ss],
                               name[ss],color=colors[ss], fontsize=10)
                elif ((last == True) & (name[ss] in fullOrbs)):
                    pylab.text(star.xanim[tt]+xshift2[ss],
                               star.yanim[tt]+yshift2[ss],
                               name[ss],color=colors[ss], fontsize=10)
        ...
        def addText():
            ...
            # Place the white border exactly on the edge:
            pylab.plot([bx, -bx+padd, -bx+padd, bx, bx], [-bx+padd, -bx+padd, bx, bx, -bx],
                       color='white', linewidth=2)
    
            #pylab.text(rad-(0.1*rad), rad-(0.2*rad), t[tt], color='white',
            pylab.text(-rad+(0.4*rad), rad-(0.2*rad), t[tt], color='white',
            ...
            # arrows look funny
            arrowSize = rad/5.
            pylab.quiver([-rad+(0.1*rad)],[-rad+(0.6*rad)],[0],[arrowSize+0.005],
                           color='w',width=0.005,scale=1,units='x')
            pylab.quiver([-rad+(0.1*rad)],[-rad+(0.6*rad)-arrowSize+0.003],[0],
                           [-arrowSize-0.005], color='w',width=0.005,scale=1,units='x')
            pylab.text(-rad+(0.25*rad), -rad+(0.55*rad), str(arrowSize)+'\"',
                       color='white', fontsize=14, fontweight='bold',
                       horizontalalignment='center', verticalalignment='top')

        if plotFullTrails == True:
            # Plot the full trail, even before discovery
            plotTrails1(extS02=False)
        else:
            # Only plot trails for time we actually observed them
            plotTrails2(extS02=False)
        ...
        pylab.savefig('%s/img_%s.png' % (outdir, str(t[tt])), dpi=dpi)

    if (extendS02 == True):
        # make a copy of the last frame but with only S0-2 and S0-102 labeled
        pylab.close(2)
        # For higher resolution, just increase figsize slightly
        pylab.figure(2, figsize=(5,5))
        pylab.clf()
        ...
        pylab.imshow(sqrt(img), origin='lowerleft', cmap=cmap,
                     ...
                     vmin=sqrt(0.01), vmax=sqrt(1.0))
        if plotFullTrails == True:
            plotTrails1(extS02=True,last=True)
        else: 
            plotTrails2(extS02=True,last=True)
        addText()
        star = gccolors.Star(0,0,0.03)
        pylab.fill(star[0], star[1], fill=False,edgecolor=sgraColor,
                   linewidth=1.5)
        pylab.axis([rad, -1.*rad, -1*rad, rad])
        pylab.savefig('%s/img_lastFrame.png' % outdir, dpi=dpi)

        
def getOrbitStars(orbitFile='',
                  root='./',
    ...
    for ii in range(len(orbs.name)):
        ...
        # First let's add in G2
        if name == 'S0-102':
            star =  s.stars[0] # dummy placeholder
...
def partiview(root='./', align='align/align_d_rms_t', poly='polyfit_d/fit'):
    #s = getOrbitStars(orbitFile='/u/ghezgroup/data/gc/source_list/orbits_movie.dat',
    s = getOrbitStars(orbitFile='', root=root, align=align, poly=poly)
...
def lgsAnimate(redoAlign=False, redoImages=False):
    ...
    imgRoots = ['05jullgs', '06maylgs1', '06junlgs', '06jullgs',
                ...
                '11maylgs', '11auglgs', '12maylgs', '12jullgs']
    ...
    vmin = array([2.48, 2.51, 2.45, 2.48, 2.50, 2.55, 2.50, 2.50, 2.50, 2.55,
                  2.53, 2.53, 2.53, 2.45, 2.45, 2.45]) 
    vmax = array([3.45, 3.32, 3.38, 3.35, 3.40, 3.25, 3.3, 3.35, 3.3, 3.30,
                  3.30, 3.30, 3.30, 3.45, 3.45, 3.45]) 
