...
def plot(labels='/u/ghezgroup/data/gc/source_list/label.dat',
        t = 2005.580, imageFile='/Users/siyao/research/work/2_align/show_ds9/mag06maylgs1_dp_msc_kp.fits',
        sgra = [1422.6, 1543.8], scale_jpg = 0.00994,vmin=1.9, vmax=6.0, mag_max=18, mag_min=-10, PA=0):
    """
    for nirc2:
    plot(labels='/Users/siyao/research/code/Ucla_source_list/label.dat',
        t = 2005.580, imageFile='/Users/siyao/research/work/2_align/show_ds9/mag06maylgs1_dp_msc_kp.fits',
        sgra = [1422.6, 1543.8], scale_jpg = 0.00994,vmin=1.9, vmax=6.0, mag_max=18, mag_min=10, PA=0):
    for HST:
    plot(labels='/Users/siyao/research/code/Ucla_source_list/label.dat',
        t = 2010, imageFile='/g/lu/data/HST/from_jay/gc/STACK.F153M.fits',
        sgra = [2005, 2007], scale_jpg=0.05,vmin=1.5, vmax=4, mag_max=16, mag_min=11, PA=0)
    
    """
    ## Read in star labels
    tab = Table.read(labels, format='ascii')
    name = tab['col1'].data
    mag = tab['col2'].data
    x0 = tab['col3'].data
    y0 = tab['col4'].data
    ...
    yim = (ypix - sgra[1]) * scale_jpg * -1.0

    # rotate an image along a point
    ...
    #    rotated = cv2.warpAffine(image, M, image.shape[0:2][::-1], flags=cv2.INTER_LINEAR)
    ...
    py.imshow(np.log10(rotate(im, PA, (int(sgra[0]), int(sgra[1])))), extent=[xim[0], xim[-1], -yim[-1], -yim[0]],
              aspect='equal', vmin=vmin, vmax=vmax, cmap=py.cm.gray)
    ...
    idx = (np.where((mag > mag_min) & (mag < mag_max) &
                    (x > xim.min()) & (x < xim.max()) &
                    ...
                    for rr in rdx:
                        ...
                        if n_aln[rr].endswith(n_lab[ss]):
                            ...
                            if np.abs(label.mag[ss] - mag[rr]) <= 0.5:
                                ...
                                foo = input('Similar names, but magnitudes are off. Check star %s manually and decide whether or not to replace it in label_updated.dat when this program completes. Hit enter to continue.' % n_aln[rr])
                        ...
                        # and was given an arbitrary name
                            ...
                            if len(rdx) > 1:
                                ...
                                for ii in range(len(rdx)):
                                    print(n_aln[rdx[ii]])
                            update = input('Update with %s velocity? (enter 1 for yes; 0 for no) ' % n_aln[rr])
                            update = int(update)
                            if update == 1:
                                out.write(fmt % \
                                          (n_lab[ss], mag[rr], x0[rr], y0[rr], x0err[rr],
