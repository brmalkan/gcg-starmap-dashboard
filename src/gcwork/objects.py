...
class Transform(object):
    ...
    def loadFromAbsolute(self,
        ...
        transTab = np.genfromtxt(root + align)

        self.numStars = transTab[idx, 1]
        self.a = [transTab[idx, 3], transTab[idx, 5], transTab[idx, 7]]
        self.b = [transTab[idx, 9], transTab[idx, 11], transTab[idx, 13]]
        self.aerr = [transTab[idx, 4], transTab[idx, 6], transTab[idx, 8]]
        self.berr = [transTab[idx, 10], transTab[idx, 12], transTab[idx, 14]]

    def loadFromAlignTrans(self, root, idx=1):
        transTab = np.genfromtxt(root + '.trans')
    ...
    def loadFromAlign(self, root):
        if os.path.exists(root + '.sgra'):
            sgraTab = np.genfromtxt(root + '.sgra')

            if len(sgraTab.shape) < 2:
                # Read old format (single row)
                self.sgra = [sgraTab[0], sgraTab[1]]
                self.sgraErr = [0.0, 0.0]
                self.angle = sgraTab[2]
                self.angleErr = sgraTab[3]
            else:
                # Read current format (multiple columns and rows).
                # But we don't use all of it... just simply and pull out a1 and a2.
                self.sgra = [sgraTab[0, 0], sgraTab[0, 1]]
                self.sgraErr = [0.0, 0.0]
                a1 = sgraTab[1, 0]
                a2 = sgraTab[2, 0]
        ...
        scaleTab = np.genfromtxt(root + '.scale')
        self.scale = float(scaleTab)
        self.scaleErr = 0.0
...
class Constants(object):
    def __init__(self):
        ...
        self.G = 6.6726e-8 # cm^3 / g / s^2
        self.msun = 1.99e33 # in grams...
        self.sec_in_yr = 3.15576e7 #365.24 days 
