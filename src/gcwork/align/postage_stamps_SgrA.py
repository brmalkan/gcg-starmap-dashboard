...
def make_postage_stamps(center_stars, align_name,
    ...
    for center_star in center_stars:
        ...
        for cur_epoch_index in tqdm(range(len(image_epochs))):
            ...
            fig = plt.figure(figsize=((16./9.)*5., 5.), frameon=False)
            ...
            image_x_bounds = [center_star_plot_x - (16./9.)*image_center_buffer, center_star_plot_x + (16./9.)*image_center_buffer]
            ...
            ax1.text((scale_xEnd - 0.5/plate_scale + scale_xEnd)/2., 5. + scale_y, '1/2 arcsec', ha='center', va='bottom', size=scale_text_size, color='white', bbox = dict(boxstyle = 'round,pad=0.25', edgecolor='none', facecolor = 'black', alpha = 0.25))
            
            ax1.text((scale_xEnd - 0.5/plate_scale + scale_xEnd)/2., scale_y - 8., '{0}'.format(int(epoch_dates[cur_epoch_index]), date_str), ha='center', va='top', size=scale_text_size, color='white', bbox = dict(boxstyle = 'round,pad=0.25', edgecolor='none', facecolor = 'black', alpha = 0.25))
            ...
            if not no_labels:
                for cur_star in neighbor_stars_sample:
                    try:
                        if label_side:
                            ax1.text(label_offset + starlist_x_pos[align_to_starlist[cur_star]], starlist_y_pos[align_to_starlist[cur_star]], cur_star.replace('irs', 'IRS '), ha='left', va='center', size='x-small', bbox = dict(boxstyle = 'round,pad=0.2', edgecolor='none', facecolor = 'white', alpha = 0.5))  # .replace('_', '\_')
                        else:
                            ax1.text(starlist_x_pos[align_to_starlist[cur_star]], label_offset + starlist_y_pos[align_to_starlist[cur_star]], cur_star.replace('irs', 'IRS '), ha='center', va='bottom', size='x-small', bbox = dict(boxstyle = 'round,pad=0.2', edgecolor='none', facecolor = 'white', alpha = 0.5))  # .replace('_', '\_')
                    except:
                        continue
                        # print(cur_star + ' not found')
            
            ## If the last epoch, label S0-2 and Sgr A*
            if label_end_frames and (cur_epoch_index == 0 or cur_epoch_index == range(len(image_epochs))[-1]):
            ...
            if make_pdfs:
                fig.savefig('{0}/{1}/pdf/PostageStamps_{1}_{2}_{3}.pdf'.format(out_dir, file_name, align_name, image_colnum), transparent=True)

            fig.savefig('{0}/{1}/png/PostageStamps_{1}_{2}_{3}.png'.format(out_dir, file_name, align_name, image_colnum), dpi=out_dpi, transparent=True)
            # with open('{0}/{1}/png/PostageStamps_{1}_{2}_{3}.png'.format(out_dir, file_name, align_name, image_colnum), 'w') as png_out_file:
            #     fig.canvas.print_png(png_out_file)
