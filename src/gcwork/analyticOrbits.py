...
from gcutil import nmpfit_sy as mpfit
...
class StarOrbitsMC(object):
    def __init__(self, theStar, ntrials=100000,
        ...
        useAcc  = if set to True, then positions, velocities, and
        	  accelerations are taken from the acceleration fit.
                  Otherwise, velocity fit is used.
        """
        self.ntrials = ntrials
        self.outroot = outroot

        self.name = theStar.name
        self.kmag = theStar.mag
        ...
        if mscStar == False:
            ...
            self.ax_dat = theStar.fitpXa.a # (+x west)
            self.ay_dat = theStar.fitpYa.a
            self.axerr_dat = theStar.fitpXa.aerr
            self.ayerr_dat = theStar.fitpYa.aerr
        else:
            starXfit = theStar.fitpXv
            starYfit = theStar.fitpYv

        # arcsec
        self.x_dat = starXfit.p # (+x west)
        ...
        # NEW
        self.sigma = 3.0 # for upper limits on accels only
        # End NEW
    ...
    def run(self, mcMassRo=None, zfrom='acc', mExtended=None, verbose=True, makeplot=False):
        ...
        # --added another generator for the case of using a z prior
        ...
        for zz in range(int(self.ntrials/2.0)):
            ...
            while(amin >= amax): # not bound; keep checking for bound condition
                ...
                if ((aminLoopCount % 100) == 2 and verbose):
                    ...
                    if (isnan(sqrt(2.0 * GM / rcgs) / 1.0e5) == 1):
                        print dist
                ...
                if mExtended != None:
                    (mass, dist, x0, y0) = self._getPotential(mcMassRo, zz, mExtended, x, y)
                else:
                    (mass, dist, x0, y0) = self._getPotential(mcMassRo, zz)

                GM = cc.G * mass * cc.msun        # cgs
                
                (x, y) = util.rPix2Arc(x, y, self.trans)     # asec (+x east, +y north)
                ...
                # Catch the case of too high velocities
                vtot = sqrt(vx**2 + vy**2 + vz**2)
                rho = sqrt(x**2 + y**2)
                vtotcgs = vtot * 1.0e5
                rhocgs = rho * dist * cc.cm_in_au

                if (vtotcgs**2 > (2.0 * GM / rhocgs)):
                    amin = -1.0
                    amax = -2.0
                    continue
                ...
                # new:
                # in case we have to use accel limits, make sure we don't
                # pull values that lead to amax being less than the accel limits
                if zfrom == 'uni_acc_limit':
                    ax = self.ax_dat
                    ay = self.ay_dat
                    axe = self.axerr_dat
                    aye = self.ayerr_dat

                    (ax, ay, axe, aye) = util.verrPix2Arc(ax, ay, axe, aye, self.trans)
                    ax *= 1000.0  # mas/yr^2
                    ay *= 1000.0  # mas/yr^2
                    axe *= 1000.0  # mas/yr^2
                    aye *= 1000.0  # mas/yr^2
                    ...
                    (ar_dat, at_dat, are_dat, ate_dat) = \
                             util.xy2circErr(-self.x_dat, self.y_dat, ax, ay,
                                             self.xerr_dat, self.yerr_dat, axe, aye)
                    alimit = ar_dat - (self.sigma * are_dat)
                    amin = alimit
                    if amin > amax:
                        # This is bad. Go back and resample
                        print 'Accel lower limit is greater than max accel. Must resample.'
                        print zz, aminLoopCount
                ...
                if (isnan(zmax) == 1):
                    ...
                    zmax = 0.0001
            #print 'sampled: ', amin, amax

            # Sample the accelerations in the plane of the sky
            # according to the distribution given by polyfit.
            # We need to check to see if this acceleration satisfies
            # a bound orbit. Search until we find a good one.
            arGood = False
            loopCnt = 0
            while (arGood == False):
                ...
                if (zfrom == 'acc'):
                    # Start NEW:
                    # We will use acceleration info only if the star passes the
                    # F test for accelerations (over velocities)
                    ...
                    if (zz == 0):
                        ...
                        print 'Significant acceleration'
                        print 'F statistic implies we should use acceleration info from polyfit.'
                    ...
                    if (zz == 0):
                        print 'Acceleration consistent w/ zero and with amin'
                        #print 'F statistic implies acceleration fit is not warranted'
                        print 'Sample from uniform acceleration between amin and amax'
                    ar = axgen.uniform(amin, amax)

                elif (zfrom == 'uni_acc_limit'):
                    if (zz == 0):
                        print 'Acceleration constraints: lower limit > amin'
                        print 'Sample from uniform acceleration between the 3 sigma lower limit and amax'

                    # This case essentially ignores amin, since we have tighter constraints
                    # using the 3 sigma lower limit on the acceleration
                    ...
                    (ax, ay, axe, aye) = util.verrPix2Arc(ax, ay, axe, aye, self.trans)
                    ax *= 1000.0  # mas/yr^2, +x to east (correct sense)
                    ay *= 1000.0  # mas/yr^2
                    axe *= 1000.0  # mas/yr^2
                    aye *= 1000.0  # mas/yr^2

                    # Convert into radial and tangential
                    # NOTE: since we're calling util.xy2circErr, must pass everything
                    # in with +x to east
                    (ar_dat, at_dat, are_dat, ate_dat) = \
                             util.xy2circErr(-self.x_dat, self.y_dat, ax, ay,
                                             self.xerr_dat, self.yerr_dat, axe, aye)
                    ...
                    ar = axgen.uniform(amin, amax)
                    #print 'sampling from: ', alimit, amax
                    
                    #print 'Sampled ar = %8.3f' % ar
                    ...
                    (ax, ay, axe, aye) = util.verrPix2Arc(ax, ay, axe, aye,
                                                          self.trans)
                    ax *= 1000.0  # mas/yr^2
                    ay *= 1000.0  # mas/yr^2
                    axe *= 1000.0  # mas/yr^2
                    ...
                    (ar, at, are, ate) = util.xy2circErr(x, y,
                                                         ax, ay,
                                                         0.0, 0.0,
                                                         axe, aye)
                    if (ar > amax):
                    ...
                    (z_dat, zerr_dat) = acc2zErr(x, y, ar, are, dist, mass)

                    if (zerr_dat > zmax):
                        # Really large errors... pull from
                        # a uniform distribution of z
                        z = axgen.uniform(0.0, zmax)
                        if (zz == 0):
                            print 'Switched to Uniform Z'
                    else:
                        z = axgen.gauss(z_dat, zerr_dat)

                    if ((z < 0) or (z >= zmax)):
                        # This is BAD. Indicate to the catcher loop
                        # below by setting acc to an unacceptable value.
                        ar = 0.0
                    else:
                        # Convert uniform z into accelerations
                        ar = z2acc(x, y, z, dist, mass)

                    if (zz == 0):
                        print 'a = %7.4f +/- %6.5f mas/yr^2' % (ar, are)
                        print 'z = %6.2f +/- %5.2f arcsec' % (z_dat, zerr_dat)
                        print 'amin = %7.4f  amax = %7.4f  zmax = %7.4f' % \
                              (amin, amax, zmax)
                    ...
                    # Get uniform acceleration
                    ar = axgen.uniform(amin, amax)
                elif (zfrom == 'all2'):
                    # Get uniform z distance
                    z = axgen.uniform(0.0, zmax)
                ...
                loopCnt += 1
                if ((loopCnt % 500) == 0 and verbose):
                    print 'Having problems getting enough accelerations within'
                    print 'the range for zz = %d (%7.3f - %7.3f) vs. %7.3f ' \
                          % (zz, amin , amax, ar)
                ...
                # Debugging:
                #print '***DEBUG:  Range of accels for zz = %d (%7.3f - %7.3f) vs. accel sampled from Gaussian:  %7.3f ' \
                #              % (zz, amin, amax, ar)
            ...
            try:
                ...
                print 'Problem calculating orbits for POSITIVE %d' % zz
                print e
                continue
            ...
            try:
                ...
                print 'Problem calculating orbits for NEGATIVE %d' % zz
                print e
                continue
        ...
        if (mcMassRo != None):
            ...
            if (mExtended != None):
                ...
                if (mExtended == 'trippe'):
                    ...
                    Mext = scipy.integrate.quad(lambda r: r**2 / (1 + (r / Rb)**2), 0, r2d)
                    Mext = const * Mext[0]
                    ...
                    const = 4.0 * math.pi * rho0
                    Mext = scipy.integrate.quad(lambda r: (r / rm)**(-gamma) * r**2, 0, r2d)
                    Mext = const * Mext[0]
...
class Disk(object):
    def __init__(self, root, mcdir, nside=64, outdir='', mscDir=None, chi2Cut=None):
        self.nside = nside
        ...
        if mscDir != None:
            ...
            yng1 = young.loadYoungStars(self.root, withRVonly=True,skipStar=['S5-237'])
            ...
            if chi2Cut != None:
                ...
                hiChiNames = [names[jj] for jj in hiChi]
                ...
                for ii in range(len(hiChi)):
                    if hiChiNames[ii] in cntrlNames:
                        ...
                        bb = np.where(np.array(names) == hiChiNames[ii])[0]
                        bad = np.concatenate([bad, bb])
                good = np.setdiff1d(np.arange(len(names)), bad)
                print
        else:
        ...
        if chi2Cut != None:
            self.names = [names[gg] for gg in good]
            self.kmag = kmag[good]
            self.r2d = r2d[good]
            self.xorig = xorig[good]
            self.yorig = yorig[good]
        else:
            self.names = names
            self.kmag = kmag
            self.r2d = r2d
    ...
    def run(self, makeplot=False, do_all=True, do_radial_bins=False,
            do_radial_bins_cntrl=False, lu09_sample=False, do_owr=False,
            do_r1=False, do_r2=False, do_r3=False, simFlat=False, do_bstars=False,
            do_nodes=False, do_r2r3=False, no_nodes=False, non_disk=False):
        ...
        iAll, oAll, r2d, names_all = self.loadStars(simFlat=simFlat, return_r2d=True)
        ...
        nodeStars = np.array(['S3-3', 'S3-5', 'S3-10', 'S3-314', 'S3-190', 'S4-36',
                              ...
                              'S7-161', 'S10-32'])
        # Temp:
        # these are just the line of node stars in the middle radial bin:
        #nodeStars = np.array(['S3-3', 'S3-5', 'S3-10', 'S3-314', 'S3-190', 'S4-36',
        #                      'S4-169', 'S5-231', 'S6-81', 'irs34W'])
        # End Temp


        if do_owr == True:
            # Only include O and WR stars (exclude B stars, identified as K>14)
            suffix = '_OWR'
            kp = []
            bad = ['S1-21','S1-22'] # produce blank PDFs in aorb_thesis ??
            print 'Making density map for O/WR stars only:'
            for ll in range(len(self.kmag)):
                if ((self.kmag[ll] >= 14.0) | (self.names[ll] in bad)):
                    print 'Skipping %s (K = %5.2f)' % (self.names[ll], self.kmag[ll])
                    continue
                else:
                    kp = concatenate([kp, [ll]])
                    print 'Keeping %s (K = %5.2f)' % (self.names[ll], self.kmag[ll])
            idx = [int(jj) for jj in kp]
                 
            i_owr = iAll[idx,:]
            o_owr = oAll[idx,:]

            # Map out the PDF for the density of normal vectors
            (neigh, neighStd, peakD, peakI, peakO) = self.densityPDF(i_owr, o_owr,
                                                                     neighbors=[6])
            # Save the i/o density maps
            print 'Making density map'
            neigh.tofile('%s/%s/disk%s.neighbor.dat' % \
                         (self.mcdir, self.outdir, suffix))
            neighStd.tofile('%s/%s/disk%s.neighborStd.dat' % \
                            (self.mcdir, self.outdir, suffix))
            peakD.tofile('%s/%s/disk%s.peakDensity.dat' % \
                         (self.mcdir, self.outdir, suffix))
            peakI.tofile('%s/%s/disk%s.peakIncli.dat' % \
                         (self.mcdir, self.outdir, suffix))
            peakO.tofile('%s/%s/disk%s.peakOmega.dat' % \
                         (self.mcdir, self.outdir, suffix))

            # Also split up into separate radial bins
            rad1 = where((r2d <= 3.197) & (self.kmag < 14.0) & (names_all != 'S1-21') & (names_all != 'S1-22'))[0]
            i_inner = iAll[rad1,:]
            o_inner = oAll[rad1,:]

            rad2 = where((r2d > 3.197) & (r2d <= 6.473) & (self.kmag < 14.0))[0]
            i_middle = iAll[rad2,:]
            o_middle = oAll[rad2,:]

            # temporary fix -- S6-95 doesn't get grouped b/c it's on the edge
            # of the middle and outer radial bins. Force it into the outer bin
            #rad3 = where(r2d > 6.473)[0]
            rad3 = where(((r2d > 6.473) & (self.kmag < 14.0)))[0]
            i_outer = iAll[rad3,:]
            o_outer = oAll[rad3,:]
            ...
            for ii in rad1:
                print '  %s (K = %4.1f)' % (self.names[ii], self.kmag[ii])
            ...
            # Save the i/o density maps
            neigh.tofile('%s/%s/inner%s_disk.neighbor.dat' % \
                     (self.mcdir, self.outdir, suffix))
            neighStd.tofile('%s/%s/inner%s_disk.neighborStd.dat' % \
                     (self.mcdir, self.outdir, suffix))
            peakD.tofile('%s/%s/inner%s_disk.peakDensity.dat' % \
                     (self.mcdir, self.outdir, suffix))
            peakI.tofile('%s/%s/inner%s_disk.peakIncli.dat' % \
                     (self.mcdir, self.outdir, suffix))
            peakO.tofile('%s/%s/inner%s_disk.peakOmega.dat' % \
                     (self.mcdir, self.outdir, suffix))

            print 'Making density map for stars at r = 3.197-6.473 arcsec'
            print '  Stars included:'
            for ii in rad2:
                print '  %s (K = %4.1f)' % (self.names[ii], self.kmag[ii])
            ...
            # Save the i/o density maps
            neigh.tofile('%s/%s/middle%s_disk.neighbor.dat' % \
                     (self.mcdir, self.outdir, suffix))
            neighStd.tofile('%s/%s/middle%s_disk.neighborStd.dat' % \
                     (self.mcdir, self.outdir, suffix))
            peakD.tofile('%s/%s/middle%s_disk.peakDensity.dat' % \
                     (self.mcdir, self.outdir, suffix))
            peakI.tofile('%s/%s/middle%s_disk.peakIncli.dat' % \
                     (self.mcdir, self.outdir, suffix))
            peakO.tofile('%s/%s/middle%s_disk.peakOmega.dat' % \
                     (self.mcdir, self.outdir, suffix))
            ...
            # Save the i/o density maps
            neigh.tofile('%s/%s/outer%s_disk.neighbor.dat' % \
                     (self.mcdir, self.outdir, suffix))
            neighStd.tofile('%s/%s/outer%s_disk.neighborStd.dat' % \
                     (self.mcdir, self.outdir, suffix))
            peakD.tofile('%s/%s/outer%s_disk.peakDensity.dat' % \
                     (self.mcdir, self.outdir, suffix))
            peakI.tofile('%s/%s/outer%s_disk.peakIncli.dat' % \
                     (self.mcdir, self.outdir, suffix))
            peakO.tofile('%s/%s/outer%s_disk.peakOmega.dat' % \
                     (self.mcdir, self.outdir, suffix))

        if do_bstars == True:
            # Only include B stars (identified as K>14)
            suffix = '_BstarsNN4'
            #suffix = '_Bstars'
            kp = []
            print 'Making density map for B stars only:'
            for ll in range(len(self.kmag)):
                if (self.kmag[ll] < 14.0):
                    print 'Skipping %s (K = %5.2f)' % (self.names[ll], self.kmag[ll])
                    continue
                else:
                    kp = concatenate([kp, [ll]])
            ...
            # Map out the PDF for the density of normal vectors
            (neigh, neighStd, peakD, peakI, peakO) = self.densityPDF(i_b, o_b,
                                                                     neighbors=[4])
                                                                     #neighbors=[6])
            # Save the i/o density maps
            print 'Making density map'
        ...
        if do_all == True:
            ...
            if no_nodes == True:
                kp = []
                print 'Making density map without node stars; keeping:'
                for ll in range(len(self.names)):
                    if self.names[ll] in nodeStars:
                        continue
                    else:
                        kp = concatenate([kp, [ll]])
                        print self.names[ll]
                ...
                # Map out the PDF for the density of normal vectors
                (neigh, neighStd, peakD, peakI, peakO) = self.densityPDF(i_node, o_node,
                                                                         neighbors=[6])
            else:
                # Map out the PDF for the density of normal vectors
        ...
        if do_radial_bins == True:
            ...
            rad1 = where(r2d <= 3.197)[0]
            i_inner = iAll[rad1,:]
            o_inner = oAll[rad1,:]

            rad2 = where((r2d > 3.197) & (r2d <= 6.473))[0]
            i_middle = iAll[rad2,:]
            o_middle = oAll[rad2,:]
            ...
            # of the middle and outer radial bins. Force it into the outer bin
            #rad3 = where(r2d > 6.473)[0]
            rad3 = where((r2d > 6.473) | (names_all == 'S6-95'))[0]
            i_outer = iAll[rad3,:]
            o_outer = oAll[rad3,:]

            # special case -- middle and outer bins, similar to Genzel+2010 review
            rad23 = where(r2d > 3.197)[0]
            i_midOuter = iAll[rad23,:]
            o_midOuter = oAll[rad23,:]

            # special case -- middle and outer bins, without stars on line of nodes
            #names_mO = names_all[rad23] # 'mO' for midOuter bins
            #rad23 = where(r2d > 3.197)[0]
            kp23 = []
            for rr in rad23:
                if names_all[rr] in nodeStars:
                    print 'Line-of-nodes star: %s' % names_all[rr]
                    continue
                else:
                    kp23 = np.concatenate([kp23, [rr]])
            kp23 = [int(k) for k in kp23]
            i_mO_noNodes = iAll[kp23,:]
            o_mO_noNodes = oAll[kp23,:]

            # remove node stars from radial bin 1
            kp1 = []
            for rr in rad1:
                if names_all[rr] in nodeStars:
                    print 'Line-of-nodes star: %s' % names_all[rr]
                    ...
                    kp1 = np.concatenate([kp1, [rr]])
            kp1 = [int(k) for k in kp1]
            i_r1_noNodes = iAll[kp1,:]
            o_r1_noNodes = oAll[kp1,:]

            # remove node stars from radial bin 2
            kp2 = []
            for rr in rad2:
                if names_all[rr] in nodeStars:
                    print 'Line-of-nodes star: %s' % names_all[rr]
                    ...
                    kp2 = np.concatenate([kp2, [rr]])
            kp2 = [int(k) for k in kp2]
            i_r2_noNodes = iAll[kp2,:]
            o_r2_noNodes = oAll[kp2,:]

            # remove node stars from radial bin 3
            kp3 = []
            for rr in rad3:
                if names_all[rr] in nodeStars:
                    print 'Line-of-nodes star: %s' % names_all[rr]
                    ...
                    kp3 = np.concatenate([kp3, [rr]])
            kp3 = [int(k) for k in kp3]
            i_r3_noNodes = iAll[kp3,:]
            o_r3_noNodes = oAll[kp3,:]

            if do_r1 == True:
                # Radial bin 1
                if no_nodes == True:
                    print
                    print 'Making density map for r < 3.197 arcsec without node stars; keeping:'
                    for ii in kp1:
                        print '  %s' % self.names[ii]
                    suffix = '_no_nodes'
                        
                    # Radial bins 2 and 3 (to make similar figure as in Genzel+2010 review)
                    # Map out the PDF for the density of normal vectors
                    (neigh, neighStd, peakD, peakI, peakO) = self.densityPDF(i_r1_noNodes, o_r1_noNodes,
                                                                             neighbors=[6])
            ...
            if do_r2 == True:
                ...
                if no_nodes == True:
                    print
                    print 'Making density map for stars at 3.197 <= r < 6.473 arcsec w/o node stars; keeping:'
                    for ii in kp2:
                        print '  %s' % self.names[ii]
                    suffix = '_no_nodes'
                    ...
                    # Radial bins 2 and 3 (to make similar figure as in Genzel+2010 review)
                    # Map out the PDF for the density of normal vectors
                    (neigh, neighStd, peakD, peakI, peakO) = self.densityPDF(i_r2_noNodes, o_r2_noNodes,
                                                                             neighbors=[6])
            ...
            if do_r3 == True:
                ...
                if no_nodes == True:
                    print
                    print 'Making density map for stars at r >= 6.473 arcsec w/o node stars; keeping:'
                    for ii in kp3:
                        print '  %s' % self.names[ii]
                    suffix = '_no_nodes'
                    ...
                    # Radial bins 2 and 3 (to make similar figure as in Genzel+2010 review)
                    # Map out the PDF for the density of normal vectors
                    (neigh, neighStd, peakD, peakI, peakO) = self.densityPDF(i_r3_noNodes, o_r3_noNodes,
                                                                             neighbors=[6])
            ...
            if (do_r2r3 == True):
                if no_nodes == True:
                    ...
                    # Radial bins 2 and 3 (to make similar figure as in Genzel+2010 review)
                    # Map out the PDF for the density of normal vectors
                    (neigh, neighStd, peakD, peakI, peakO) = self.densityPDF(i_mO_noNodes, o_mO_noNodes,
                                                                             neighbors=[6])
                    ...
                    # Radial bins 2 and 3 (to make similar figure as in Genzel+2010 review)
                    # Map out the PDF for the density of normal vectors
                    (neigh, neighStd, peakD, peakI, peakO) = self.densityPDF(i_midOuter, o_midOuter,
                                                                             neighbors=[6])
                
                ## Save the i/o density maps
                neigh.tofile('%s/%s/midOuter%s_disk.neighbor.dat' % \
                             (self.mcdir, self.outdir, suffix))
                neighStd.tofile('%s/%s/midOuter%s_disk.neighborStd.dat' % \
                                (self.mcdir, self.outdir, suffix))
                peakD.tofile('%s/%s/midOuter%s_disk.peakDensity.dat' % \
                             (self.mcdir, self.outdir, suffix))
                peakI.tofile('%s/%s/midOuter%s_disk.peakIncli.dat' % \
                             (self.mcdir, self.outdir, suffix))
                peakO.tofile('%s/%s/midOuter%s_disk.peakOmega.dat' % \
                             (self.mcdir, self.outdir, suffix))
        ...
        if simFlat == True:
            # Only grab the disk candidate stars
            # Get disk membership
            diskTab = asciidata.open(self.root+'tables/disk_membership_prob_mosaic.dat')
            nameTmp = [diskTab[0][ss].strip() for ss in range(diskTab.nrows)]
            diskP = diskTab[1].tonumpy()
            didx = where(diskP > 2.7e-3)[0] # candidates
            diskStars = [nameTmp[nn] for nn in didx]

            idx = []
            for ii in range(len(didx)):
                kp = self.names.index(diskStars[ii])
                idx = concatenate([idx, [kp]])
            ...
            # Map out the PDF for the density of normal vectors
            (neigh, neighStd, peakD, peakI, peakO) = self.densityPDF(i_disk, o_disk,
                                                                     neighbors=[6])
            
            # Save the i/o density maps
            print 'Making density map assuming stars are on disk'
        ...
        if do_nodes == True:
            ...
            # Selected as clockwise moving stars that are within 1" from line of
            # nodes, and beyond r=3.2", which is the middle and outer radial bins
            idx = []
            for ll in nodeStars:
                l = np.where(np.array(self.names) == ll)[0]
                idx = concatenate([idx, l])
            idx = [int(ll) for ll in idx]
            ...
            # Save the i/o density maps
            print 'Making density map for stars apparently near line of nodes'
            neigh.tofile('%s/%s/lineOfNodes_disk.neighbor.dat' % \
                         (self.mcdir, self.outdir))
            neighStd.tofile('%s/%s/lineOfNodes_disk.neighborStd.dat' % \
                            (self.mcdir, self.outdir))
            peakD.tofile('%s/%s/lineOfNodes_disk.peakDensity.dat' % \
                         (self.mcdir, self.outdir))
            peakI.tofile('%s/%s/lineOfNodes_disk.peakIncli.dat' % \
                         (self.mcdir, self.outdir))
            peakO.tofile('%s/%s/lineOfNodes_disk.peakOmega.dat' % \
                         (self.mcdir, self.outdir))
        ...
        if non_disk == True:
            ...
            diskTab = asciidata.open('/u/syelda/research/gc/aligndir/11_10_26/tables/disk_membership_prob_mosaic.dat')
            ...
            idx = []
            for nn in nondisk:
                print 'Non-disk star: %s' % nn
                n = np.where(np.array(self.names) == nn)[0]
                idx = concatenate([idx, n])
            idx = [int(ii) for ii in idx]
            ...
            # Save the i/o density maps
            print 'Making density map for stars not on the disk'
            neigh.tofile('%s/%s/non_disk.neighbor.dat' % \
                         (self.mcdir, self.outdir))
            neighStd.tofile('%s/%s/non_disk.neighborStd.dat' % \
                            (self.mcdir, self.outdir))
            peakD.tofile('%s/%s/non_disk.peakDensity.dat' % \
                         (self.mcdir, self.outdir))
            peakI.tofile('%s/%s/non_disk.peakIncli.dat' % \
                         (self.mcdir, self.outdir))
            peakO.tofile('%s/%s/non_disk.peakOmega.dat' % \
                         (self.mcdir, self.outdir))
    ...
    def sample_wr(self, samples, rbin):
        ...
        iAll, oAll, r2d, names_all = self.loadStars(return_r2d=True)
    ...
    def loadStars(self, simFlat=False, return_r2d=False):
        ...
        for ss in range(nstars):
            ...
            try:
                ...
                if simFlat == True:
                    _f = open('%s/%s.disk_mc.dat' % (self.mcdir, name), 'r')
                else:
        ...
        if return_r2d:
            return iAll, oAll, self.r2d, names_all
        else:
            return iAll, oAll

    def densityPDF(self, iAll, oAll, neighbors=[4,5,6,7], aperture=False,
                   sampleWR=None):
        """
        Map out the PDF for the density of normal vectors
        """
...
class simulate_flat_disk(object):
    def __init__(self, theStar, ntrials=100000,
                 outroot='flatdisk/', zDisk=None, mscStar=False):
        """
        Simulate a flat disk -- assumes the line of sight distance that puts
        the star on the disk. Then samples from the other 5 kinematic variables
        and their errors, and runs the orbits. This will tell us what the
        contribution is from the errors in the 5 kinematic variables
        assuming a flat disk.
        ...
        # Get the z that puts this star on the disk
        self.zdisk = zDisk

    def runmc(self, mcMassRo=None, mExtended=None,
        ...
        gens = create_generators(5, self.ntrials*1000)
        xgen = gens[0]
        ygen = gens[1]
        vxgen = gens[2]
        vygen = gens[3]
        ...
        for zz in range(int(self.ntrials)):
            ...
            # amin (set by r2d) < amax (set by v=vesc), where
            # all accels are negative.
            while(vgood == False): # not bound; keep checking for bound condition
                loopCount += 1
                
                # Occasionally we get cases where the randomly selected
                # velocity exceeds the escape velocity for the randomly
                # selected distance. We need to throw out these cases.
                if ((loopCount % 100) == 2 and verbose):
                    r = sqrt(x**2 + y**2)             # arcsec
                    rcgs = r * dist * cc.cm_in_au
                ...
                vtot = sqrt(vx**2 + vy**2 + vz**2)
                #rho = sqrt(x**2 + y**2)
                # Use 3D distance, since we have it
                rho = sqrt(x**2 + y**2 + self.zdisk**2)
                vtotcgs = vtot * 1.0e5
                rhocgs = rho * dist * cc.cm_in_au
            ...
            zidx = zz
            try:
                self._runOrbit(zidx, x, y, z, vx, vy, vz, mass, dist, x0, y0)
            except ValueError, e:
                print 'Problem calculating orbits for %d' % zz
                print e
                continue
    ...
    def _getPotential(self, mcMassRo, zz, mExtended, x=None, y=None):
        cc = objects.Constants()
    ...
    def _runOrbit(self, zidx, x, y, z, vx, vy, vz, mass, dist, x0, y0):
        rvec = array([x, y, z])
        vvec = array([vx, vy, vz])
        revec = zeros(3, dtype=float)
        vevec = zeros(3, dtype=float)
	...
	pdf /= self.ntrials
        ...
        if (makeplot):
            mcFile = '%s%s_disk_mc_heal.dat' % (self.outroot, self.name)
            pdf.tofile(mcFile)
...
class IsotropicMC(object):
    def __init__(self, ntrials, pdftrials):
        ...
        root = '/u/syelda/research/gc/aligndir/11_10_26/'
        mscDir = '/u/syelda/research/gc/aligndir/12_01_30_dp_msc/'
        polyM = 'polyfit_1000/fit' 
        pointsM='points_1000/'
        self.doMassRo = pickle.load( open(root + 'tables/massRo_111026.dat') )
        self.mExtended = pickle.load( open(root + 'tables/rho0_schoedel09.dat') ) 

        #yng = young.loadYoungStars(root, withRVonly=True)
        # Load names of young stars 
        # Load up mosaic data as well; select only stars at r>4, since
        # we don't want to add any info from mosaics if we have it in
        # the central 10" already
        yng1 = young.loadYoungStars(root, withRVonly=True)
        yng2 = young.loadYoungStars(mscDir,fit=polyM, withRVonly=True,
                                    points=pointsM, mosaic=True)
        self.cntrlNames = yng1.getArray('name')
        self.mscNames = yng2.getArray('name')
        # Merge this object with object from central 10" analysis
        yng = merge(yng1, yng2)
    ...
    def runmc(self, savefile, verbose=False, zfrom='all'):
        ...
        for ii in range(self.ntrials):
            ...
            for ss in range(self.numStars):
                star = self.stars[ss]
                mscStar = False
                if (star.name in self.mscNames) & (star.name not in self.cntrlNames):
                    mscStar = True
                ...
                # Randomly sample the direction of the velocity vector
                vxmax = star.vmag
                ...
                mc = StarOrbitsMC(star, ntrials=self.pdftrials,
                                  outroot='iso_mc/',mscStar=mscStar)
                mc.run(verbose=False, mcMassRo=self.doMassRo, zfrom=zfrom,
                       mExtended=self.mExtended)
...
def diskWidthCDF(mapfile, n, nside=64):
    ...
    n -- Which of the maps to use from the stack (typically n=2 for
         the 6th nearest neighbor analysis.
    """
    npix = healpy.nside2npix(nside)
    #npix = healpix.nside2npix(nside)
...
def plane2z(x, y, idisk=130.2, odisk=96.3):
    """
